...
from .lbs import lbs, lbs_ik, rotmat_to_quat, quat_to_rotmat
...
class SMIL_layer(nn.Module):
    ...
    def __init__(self,
        ...
        '''
        super(SMIL_layer, self).__init__()

        self.ROOT_IDX = self.JOINT_NAMES.index('pelvis')
        self.LEAF_IDX = [self.JOINT_NAMES.index(name) for name in self.LEAF_NAMES]
        self.SPINE3_IDX = 9

        with open(model_path, 'rb') as smil_file:
            self.smil_data = Struct(**pk.load(smil_file, encoding='latin1'))

        self.gender = gender

        self.dtype = dtype

        self.faces = self.smil_data.f

        ''' Register Buffer '''
        # Faces
        self.register_buffer('faces_tensor',
                             to_tensor(to_np(self.smil_data.f, dtype=np.int64), dtype=torch.long))

        # The vertices of the template model, (6890, 3)
        self.register_buffer('v_template',
                             to_tensor(to_np(self.smil_data.v_template), dtype=dtype))

        # The shape components
        # Shape blend shapes basis, (6890, 3, 10)
        self.register_buffer(
            'shapedirs',
            to_tensor(to_np(self.smil_data.shapedirs), dtype=dtype))

        # Pose blend shape basis: 6890 x 3 x 23*9, reshaped to 6890*3 x 23*9
        num_pose_basis = self.smil_data.posedirs.shape[-1]
        # 23*9 x 6890*3
        posedirs = np.reshape(self.smil_data.posedirs, [-1, num_pose_basis]).T
        ...
        self.register_buffer(
            'J_regressor',
            to_tensor(to_np(self.smil_data.J_regressor), dtype=dtype))
        ...
        parents[:(self.NUM_JOINTS + 1)] = to_tensor(to_np(self.smil_data.kintree_table[0])).long()
        parents[0] = -1
        ...
        self.register_buffer('lbs_weights',
                             to_tensor(to_np(self.smil_data.weights), dtype=dtype))
    ...
    def forward(self,
                ...
                transl=None,
                return_verts=True):
        ''' Forward pass for the SMIL model
        ...
        if transl is not None:
            ...
            joints = joints - joints[:, self.root_idx_smil, :].unsqueeze(1).detach()
    ...
    def Inverse_Kinematics(
        ...
        ''' Inverse pass for the SMIL model
        ...
        vertices, new_joints, rot_mats, joints_from_verts = lbs_ik(
        ...
        if transl is not None:
            ...
            new_joints = new_joints - new_joints[:, self.root_idx_smil, :].unsqueeze(1).detach()
