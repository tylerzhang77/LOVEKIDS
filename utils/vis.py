...
def vis_bbox(image, bbox):
    ...
    bbox_img = cv2.rectangle(image, (int(x1), int(y1)),
...
def vis_frame(frame, im_res, tracking, format='coco', add_bbox=False, skeleton=None):
    ...
    for human in im_res:
        part_line = {}
        keypoints = torch.tensor(human['pred_xyz_jts_17_flat']).reshape(-1, 3)
        ...
        for n in range(kp_scores.shape[0]):
            ...
            if n < len(p_color):
                ...
                if n == 70:
                    cv2.circle(bg, (int(cor_x), int(cor_y)),
                               1, (255, 136, 132), 2)
                else:
                    cv2.circle(bg, (int(cor_x), int(cor_y)),
                               1, (255, 255, 255), 2)
        ...
        for i, (start_p, end_p) in enumerate(l_pair):
            if start_p in part_line and end_p in part_line:
                ...
                transparency = float(
                    max(0, min(1, min(kp_scores[start_p], kp_scores[end_p]))))
                img = cv2.addWeighted(
        ...
        if add_bbox:
            ...
            xmin, ymin, xmax, ymax = int(
                xmin + 0.5), int(ymin + 0.5), int(xmax + 0.5), int(ymax + 0.5)
...
def vis_uvd_trivial(uvd_data, uvd_weight=None, imgs=None, idx=0, dataset_name='3dhp', rescale=False, saved_path=None):
    ...
    for i in range(batch):
        ...
        new_img = vis_frame(img[:, :, ::-1], item_dict,
                            tracking=False, format='h36m')
